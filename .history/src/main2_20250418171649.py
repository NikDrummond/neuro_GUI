...
class MainWindow(QMainWindow):
    ...
    def toggle_select_points(self, on):
        iren = self.vtkWidget.GetRenderWindow().GetInteractor()
        if on:
            # Initialize our special point‐selection overlay
            ...
            # Remove the hover/click observers
    ...
    def _on_hover(self, caller, event):
        ...
        idx   = dists.argmin()
        if dists[idx] > SOME_THRESHOLD:  # optional: only if very close
            return
        # temporary highlight
    ...
    def _on_click(self, caller, event):
        ...
        dists   = np.linalg.norm(self.pnt_coords - pickPos, axis=1)
        idx     = dists.argmin()
        if dists[idx] > SOME_THRESHOLD:
            return
        # flip that bit in the mask
        self.pnt_mask[idx] = True
        # rebuild & re‐plot
        self.plotter.remove(self.pnt_in).remove(self.pnt_out)
        self.pnt_in, self.pnt_out = make_pnts(self.pnt_coords, self.pnt_mask)
        self.plotter.add(self.pnt_out).add(self.pnt_in)
        self.plotter.render()
