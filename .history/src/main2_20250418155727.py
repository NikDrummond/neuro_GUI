...
ios.environ['LD_LIBRARY_PATH'] = '/usr/lib/x86_64-linux-gnu:' + os.environ.get('LD_LIBRARY_PATH','')
...
class MainWindow(QMainWindow):
    ...
    def _update_scale_overlay(self, caller=None, event=None):
        # compute pixel length for world length
        ren = self.plotter.renderer
        # choose two points on focal plane
        fp = ren.GetActiveCamera().GetFocalPoint()
        # world points: origin at FP, second offset along camera view right direction
        # but approximate using world X-axis
        p1 = (fp[0], fp[1], fp[2], 1)
        length_world = self.scale_length_nm  # in nm
