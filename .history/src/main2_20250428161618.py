...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # initialise empty point coordinates
        self.pnt_coords = np.array([[0,0,0],[1,1,1]])
        ...
        side_layout.addWidget(self.mask_down_btn)
    ...
    def toggle_select_points(self, on):
        ...
        if on:
            ...
            if not hasattr(self, 'pnt_mask'):
                ...
                self._update_mask_botton_state()
            else:
    ...
    def _update_mask_button_state(self):
        """
        Enable 'Mask Downstream' only if:
          - we're in select‐points mode, and
          - exactly one point is selected in self.pnt_mask
        """
        enabled = (
            self.select_checkbox.isChecked() and
            self.pnt_mask is not None and
            np.sum(self.pnt_mask) == 1
        )
        self.mask_down_btn.setEnabled(enabled)

    def mask_downstream(self):
        """
        Stub: called when exactly one point is selected.
        Here you can look at which index is True in self.pnt_mask
        and implement your downstream‐masking logic.
        """
        idx = int(np.nonzero(self.pnt_mask)[0])
        # … your masking code here …
        logging.info(f"Masking downstream from point {idx}")    
