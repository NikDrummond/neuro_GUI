...
class QTextEditLogger(logging.Handler):
    """A logging handler that sends logs to a QTextEdit widget."""
    def __init__(self, text_edit: QTextEdit):
        super().__init__()
        self.widget = text_edit
        # optional: set a simple format
        fmt = logging.Formatter('%(asctime)s: %(levelname)s: %(message)s', "%H:%M:%S")
        self.setFormatter(fmt)

    def emit(self, record):
        msg = self.format(record)
        # append in the GUI thread
        def append():
...
class MainWindow(QMainWindow):
    ...
    def _init_menu(self):
        ...
        self.log_dock.hide()  # hidden by default

        show_log_act = QAction("Show Log", self)
        show_log_act.setCheckable(True)
        show_log_act.toggled.connect(self._toggle_log_dock)
        tools_menu.addAction(show_log_act)

        # Install our QTextEdit logger into the root logger
        qt_logger = QTextEditLogger(self.log_console)
        logging.getLogger().addHandler(qt_logger)
        logging.getLogger().setLevel(logging.DEBUG)  # or your preferred level
        # Viewer
