...
class MainWindow(QMainWindow):
    ...
    def toggle_select_points(self, on):
        ...
        if on:
            ...
            # Exiting: remove hover/click callbacks
            if hasattr(self, 'hover_obs'):
                iren.RemoveObserver(self.hover_obs)
            if hasattr(self, 'click_obs'):
                iren.RemoveObserver(self.click_obs)
            # and clear the lead/branch point actors
    ...
    def _on_hover(self, caller, event):
        # gracefully ignore bad picks
        pickPos = caller.GetPicker().GetPickPosition()
        # find nearest lead/branch point
        dists = np.linalg.norm(self.pnt_coords - pickPos, axis=1)
        idx   = dists.argmin()
        if dists[idx] > 100:  # optional: only if very close
            return
        # temporary highlight
        tmp = vd.Point(self.pnt_coords[idx], c='yellow', r=15)
        self.plotter.add(tmp); self.plotter.render(); self.plotter.remove(tmp)

    def _on_click(self, caller, event):
        # pick a world‐space position under cursor
        x, y = caller.GetEventPosition()
        self.picker.Pick(x, y, 0, self.plotter.renderer)
        pick_pos = self.picker.GetPickPosition()

        # find nearest index in your lead/branch coords
        dists = np.linalg.norm(self.pnt_coords - pick_pos, axis=1)
        idx = dists.argmin()
        # optional: ignore clicks farther than some threshold
        # if dists[idx] > 5: return

        # flip that mask entry
        self.pnt_mask[idx] = True

        # remove old actors, rebuild and re‐add
        self.plotter.remove(self.pnt_in)
        self.plotter.remove(self.pnt_out)
        self.pnt_in, self.pnt_out = make_pnts(self.pnt_coords, self.pnt_mask)
        self.plotter.add(self.pnt_out).add(self.pnt_in)
